<?php
    ...
    if (!$sanitizer->validateResetToken($token)) {
        logs_webapp("invalid format (token)", getClientIP(), "password_recovery.log");
        http_response_code(400);
    ...
    if (!$sanitizer->validatePassword($password)) {
        logs_webapp("invalid format (password)", getClientIP(), "password_recovery.log");
        http_response_code(400);
    ...
    if (password_verify($password, $oldpassword)){
        logs_webapp("tried to reuse the old password during password recovery for user $userId", getClientIP(), "password_recovery.log");
        http_response_code(400);
    ...
    if ($updateStmt->execute()) {
        ...
        if ($deleteStmt->execute()) {
            $pdo->commit();
            logs_webapp("changed password for user $userId", getClientIP(), "password_recovery.log");
        ...
        logs_webapp("something gone wrong during password update for user $userId", getClientIP(), "password_recovery.log");
